...
| ✅ | 발표 연습 설정 - 폴더 내 자료 목록 조회 | `GET /practice-folders/{folderId}/presentation-practices` | 없음 | `practices: [{ practiceId, presentationId, title, description, durationSec, createdAt }]` |

폴더 화면은 실제 API 응답만 사용합니다. 선택·생성한 `folderId`는 발표·면접 세션 생성 요청에 숫자 ID로 전달하며, 과거 목 ID가 세션에 남아 있으면 요청 전에 폐기합니다.

전용 `/materials` API는 없어서, 자료 선택 카드는 이미 있는 `/presentation-practices` 응답을 `practiceApi.listFolderMaterials()`에서 `{ id: presentationId, name: title, type: null, uploadedAt: createdAt }`로 매핑해 재사용합니다. 서버가 원본 파일 종류(PPTX/PDF)를 안 내려줘서 `type`은 항상 `null`이고, 화면에서는 그 배지를 숨깁니다.
...
| ✅ | 폴더 기존 자료 재사용해 발표 생성 | `POST /presentations/reuse` | `folderId`, `sourcePresentationId`, `title`, `description`, `targetDurationSec`, `aiQnaEnabled` (파일 없음) | `presentationId`, `practiceId`, `status`(실제 응답은 `PENDING`으로 옴) |
...
`/presentations/reuse`는 `sourcePresentationId`가 **같은 폴더 안에서 이미 완료(`COMPLETED`)된 발표**여야 합니다. 미완료 자료를 재사용하려 하면 서버가 에러로 거부합니다(`PRESENTATION_NOT_COMPLETED` 등). 실제 응답은 `status: "PENDING"`으로 오므로, `/presentations`와 동일하게 생성 후 `GET /presentations/{presentationId}/status`를 폴링해 `COMPLETED`가 된 뒤 슬라이드를 조회합니다.
