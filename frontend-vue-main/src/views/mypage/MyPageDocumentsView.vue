...
const validateForm = () => {
  ...
  if (title.length > INPUT_LIMITS.DOCUMENT_TITLE) return `자료 제목은 ${INPUT_LIMITS.DOCUMENT_TITLE}자 이하로 입력해 주세요.`
  return validateSupportDocumentFile(documentFile.value)
}
    ...
    <div v-if="registrationOpen" class="doc-modal-backdrop" @click.self="closeRegistration">
      ...
      >
        ...
        <form v-else data-testid="submit-document" @submit.prevent="submitRegistration">
          ...
          <label class="doc-form-field">
            <span>자료 제목</span>
            <div class="limited-field is-inline-field">
              <input v-model="documentTitle" data-testid="document-title" type="text" :maxlength="INPUT_LIMITS.DOCUMENT_TITLE" placeholder="자료 제목을 입력해 주세요" :disabled="store.loading" />
              <small class="field-counter">{{ documentTitle.length }}/{{ INPUT_LIMITS.DOCUMENT_TITLE }}</small>
            </div>
          </label>
