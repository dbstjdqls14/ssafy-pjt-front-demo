...
const onNext = async () => {
  if (mode.value === 'new') {
    ...
    if (newFolderName.value.trim().length > FOLDER_NAME_MAX_LENGTH) {
      nameError.value = `폴더명은 ${FOLDER_NAME_MAX_LENGTH}자 이하로 입력해주세요. (현재 ${newFolderName.value.trim().length}자)`
      return
    }
              ...
              <div class="folder-new-form">
                <div class="form-field" :class="{ 'field-invalid': nameError }">
                  ...
                  <div class="limited-field is-underline-field">
                    <input id="newFolderName" v-model="newFolderName" type="text" :maxlength="FOLDER_NAME_MAX_LENGTH" placeholder="예) 졸업작품 발표" @input="nameError = ''" />
                    <small class="field-counter" data-testid="folder-name-counter">{{ newFolderName.length }}/{{ FOLDER_NAME_MAX_LENGTH }}</small>
                  </div>
                ...
                <div class="form-field">
                  ...
                  <div class="folder-desc-field limited-field">
                    <textarea id="newFolderDesc" v-model="newFolderDesc" rows="4" :maxlength="FOLDER_DESC_MAX_LENGTH" placeholder="이 주제에 대한 간단한 메모를 남겨보세요."></textarea>
                    <span class="folder-desc-counter">{{ newFolderDesc.length }}/{{ FOLDER_DESC_MAX_LENGTH }}</span>
                  </div>
                </div>
              </div>
            </div>
