...
import { practiceTitleValidationMessage, stripEmoji } from '../../utils/validators.js'
...
const onTitleInput = (event) => {
  const cleaned = stripEmoji(event.target.value)
  title.value = cleaned
...
const goNext = async () => {
  ...
  if (description.value.trim().length > DESCRIPTION_MAX_LENGTH) {
    descriptionError.value = `연습 설명은 ${DESCRIPTION_MAX_LENGTH}자 이하로 입력해주세요. (현재 ${description.value.trim().length}자)`
    return
  }
            ...
            >
              ...
              <div class="form-field" :class="{ 'field-invalid': titleError }">
                ...
                <div class="limited-field is-inline-field">
                  <input
                    ...
                    @input="onTitleInput"
                  />
                  <small class="field-counter" data-testid="presentation-title-counter">{{ title.length }}/{{ TITLE_MAX_LENGTH }}</small>
                </div>
              ...
              <div class="form-field" :class="{ 'field-invalid': descriptionError }">
                ...
                <div class="limited-field">
                  ...
                  ></textarea>
                  <small class="field-counter" data-testid="presentation-description-counter">{{ description.length }}/{{ DESCRIPTION_MAX_LENGTH }}</small>
                </div>
