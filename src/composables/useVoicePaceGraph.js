...
export function useVoicePaceGraph(voicePaceRef, durationSecRef) {
  const paceChartEl = ref(null)

  const xForSec = (sec) => (sec / durationSecRef.value) * 600
  const pcOfSec = (sec) => Math.min(100, Math.max(0, (sec / durationSecRef.value) * 100))
  ...
  const paceYBounds = computed(() => {
    ...
    const lo = Math.min(...paces, Number.isFinite(avgPace) ? avgPace : paces[0]) - 0.4
    const hi = Math.max(...paces, Number.isFinite(avgPace) ? avgPace : paces[0]) + 0.4
    return { lo, hi }
  })
  const paceYFor = (pace) => {
    ...
    return [
      {
        ...
        pace: `초당 ${vp.slowest.pace.toFixed(1)}음절`,
      ...
      },
      {
        ...
        pace: `초당 ${vp.fastest.pace.toFixed(1)}음절`,
  ...
  // 필러 점: 계단 그래프는 구간마다 y값이 고정돼 있어(버킷 조회) DOM 경로
