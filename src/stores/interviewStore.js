...
export const useInterviewStore = defineStore('interview', () => {
  ...
  const questions = ref(draft.questions ?? [])
  ...
  // TTS(질문 읽어주는 음성) 재생 구간 — 영상의 실제 경과 시간(videoElapsedSeconds)
  // 기준 [{questionIndex,start,end}]. 백엔드엔 없는, 이번 세션에서 방금 녹화한
  // 것에만 있는 로컬 전용 데이터라 새로고침·재조회 시엔 사라진다 — 리포트
  // 화면은 이 값이 있을 때만 영상 재생 시각에 맞춰 TTS 구간 표시·자막 보정을 한다.
  const sessionTtsWindows = ref([])
  ...
  watch(
    [interviewId, practiceId, title, description, companyId, company, occupationId, field, jobId, position,
      ...
      createdSignature, recordedSeconds, answers],
    () => {
  ...
  const setInfo = (patch) => {
    ...
    if (patch.description !== undefined) description.value = String(patch.description ?? '').slice(0, 50)
  ...
  const loadQuestions = async (id = interviewId.value) => {
    ...
    questions.value = normalizeInterviewQuestions(readApiCollection(response, ['questions', 'items', 'content']))
  ...
  const createInterview = async ({ force = false } = {}) => {
    ...
    try {
      ...
      questions.value = normalizeInterviewQuestions(value.questionItems ?? value.questions ?? [])
  ...
  const finishRecording = ({ videoBlob = null, audioBlob = null, blob = null, durationSeconds = 0, answers: nextAnswers = [], nonverbal = null, ttsWindows = [] }) => {
    ...
    sessionTtsWindows.value = ttsWindows
  ...
  const beginAnalysis = async () => {
    ...
    analysisProgress.value = 0
    analysisError.value = ''
    completePromise = completeInterview()
      .then(() => {
  ...
  const loadReport = async (id) => {
    ...
    if (!report.value && targetId) {
      report.value = unwrapApiResponse(await interviewApi.getReport(targetId))
    }
    return report.value
  }
  ...
  return {
    ...
    recordedSeconds, answers, sessionAudioBlob, sessionVideoBlob, sessionNonverbal, sessionTtsWindows,
    analysisStatus, analysisProgress, analysisError, report,
