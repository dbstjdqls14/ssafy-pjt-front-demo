...
const onSubmit = async () => {
  ...
  try {
    ...
    errors.value.form = caught?.status === 401 || caught?.status === 400
      ? '이메일 또는 비밀번호를 확인해주세요.'
...
<template>
  <main class="auth-wrap auth-editorial">
    <section class="auth-card" aria-labelledby="loginTitle">
      ...
      <button type="button" class="social-btn kakao" @click="errors.form = '소셜 로그인은 준비 중이에요. 이메일로 로그인해주세요.'">
        <img class="social-icon" :src="kakaoIcon" alt="" aria-hidden="true" />
        <span>카카오로 계속하기</span>
      </button>
      <button type="button" class="social-btn google" @click="errors.form = '소셜 로그인은 준비 중이에요. 이메일로 로그인해주세요.'">
