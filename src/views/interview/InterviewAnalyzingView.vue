    ...
    <section class="analysis-status-panel">
      ...
      <div class="analysis-status-content">
        <template v-if="failed">
          <h3 style="font-size:16px;font-weight:900;">면접 분석을 완료하지 못했습니다.</h3>
          <p class="analysis-error-message" role="alert">{{ interview.analysisError }}</p>
          <button type="button" class="btn-primary analysis-retry-button" @click="retry">다시 분석하기</button>
        </template>
        <template v-else>
          <h3 style="font-size:16px;font-weight:900;">답변 {{ total }}개 중 {{ analyzed }}개 분석 완료</h3>
          <p style="margin-top:4px;color:#94a3b8;font-size:12px;font-weight:700;">서버 분석 상태를 확인하고 있어요.</p>
          ...
          <p style="margin-top:14px;color:#94a3b8;font-size:11px;font-weight:600;">이 화면을 나가도 서버에서 분석은 계속 진행됩니다.</p>
        </template>
      </div>
    </section>
  </main>
