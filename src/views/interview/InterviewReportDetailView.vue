...
import { questionIndexAtTime } from '../../utils/interviewTimeline.js'
...
const totalRecordingDurationSec = computed(() => {
  ...
  const totalTtsShift = questions.value.length ? ttsShiftAt(questions.value.length - 1) : 0
  return Number.isFinite(reported) && reported > 0
    ? reported + totalTtsShift
...
onBeforeUnmount(() => {
  if (voiceVideoObjectUrl) URL.revokeObjectURL(voiceVideoObjectUrl)
...
// 실제 측정 가능한 지표만 남긴다 — 음성(필러·말 속도·침묵), 몸짓(시선 이탈·기울기).
const scoreCards = computed(() => {
  // 음성 지표는 필러·말 속도·침묵만 노출한다. 백엔드가 '말 더듬음'·'목소리 떨림'
  ...
  return normalizeReportScoreCards(session.value.scoreCards, []).map((card) => {
    ...
    // 백엔드의 "전체 인터뷰" 필러/침묵 합계가 질문별 합계와 어긋나는 경우가
    ...
    if (questions.value.some((q) => q.voicePace)) {
      ...
      const fillerIndex = rows.findIndex(([rowLabel]) => rowLabel === '필러')
      if (fillerIndex !== -1) rows[fillerIndex] = ['필러', `${sumQuestionMetric('fillerTotal')}회`]
...
const normalizeCaptionSegments = (segments) => (
  Array.isArray(segments)
    ? segments
      .map((s) => ({ start: Number(s.start ?? s.startSec ?? 0), end: Number(s.end ?? s.endSec ?? 0), text: String(s.text ?? '').trim() }))
      .filter((s) => s.text)
    : []
)
...
// TTS(질문을 읽어주는 음성) 재생 중엔 답변 오디오·자막은 멈추지만 영상은 계속
// 녹화된다 — 그래서 백엔드 시각(답변만 기준, "압축된" 시간)과 영상의 실제
// 재생 시각이 질문이 넘어갈수록 점점 벌어진다. 방금 이 브라우저에서 녹화한
// 세션이라면(interview.sessionTtsWindows) 질문별 TTS 재생 길이를 알고 있으니,
// 그 누적값만큼 뒤 질문들의 시각을 밀어서 영상 실제 재생 시각과 맞춘다.
// 서버에서 다시 불러온 예전 리포트는 이 정보가 없어 보정 없이 그대로 쓴다.
const ttsShiftByQuestion = computed(() => {
  const windows = interview.sessionTtsWindows ?? []
  if (!windows.length) return []
  const count = Array.isArray(session.value.questions) ? session.value.questions.length : 0
  const shifts = []
  let running = 0
  for (let i = 0; i < count; i += 1) {
    running += windows
      .filter((w) => w.questionIndex === i)
      .reduce((sum, w) => sum + Math.max(0, Number(w.end) - Number(w.start)), 0)
    shifts.push(running)
  }
  return shifts
})
const ttsShiftAt = (index) => {
  const shifts = ttsShiftByQuestion.value
  if (!shifts.length) return 0
  return shifts[Math.min(index, shifts.length - 1)] ?? 0
}
const ttsShiftBefore = (index) => (index <= 0 ? 0 : ttsShiftAt(index - 1))
// 질문 index의 TTS 재생 구간을 영상 실제 시각 기준으로 — 그 질문의 답변이
// 시작하는 시각 바로 앞, 이전까지 밀린 만큼 뒤 duration만큼의 구간이다.
const ttsWindowVideoRangeAt = (index) => {
  const before = ttsShiftBefore(index)
  const at = ttsShiftAt(index)
  return at > before ? { start: before, end: at } : null
}
...
const questions = computed(() => {
  ...
  return source.map((q, index) => {
    ...
    const rawDurationSec = Number(
      ...
      ?? item.answerDurationSeconds
      ?? (Number.isFinite(explicitStartSec) && Number.isFinite(explicitEndSec) ? explicitEndSec - explicitStartSec : 0),
    )
    const durationSec = Number.isFinite(rawDurationSec) && rawDurationSec > 0 ? rawDurationSec : 1
    ...
    const startOffset = rawStartOffset + ttsShiftAt(index)

    const captionSegments = normalizeCaptionSegments(item.segments)
    const voicePace = normalizeVoicePace(item.voicePace)

    const localGestureSeries = sliceLocalGestureSeries(interview.sessionNonverbal, startOffset, durationSec)
    const gestureSeries = localGestureSeries ?? normalizeGestureSeries(item.gestureSeries)
    const answer = String(item.answer ?? item.transcript ?? '')
...
const EMPTY_QUESTION = {
  ...
  metric: '', feedback: '', startSec: 0, durationSec: 1, captionSegments: [],
...
const currentDurationSec = computed(() => Math.max(1, current.value.durationSec))
...
// 우측 자막은 그래프와 달리 "전체 녹화" 기준 — 질문을 넘겨도 이전 질문에서
// 지나온 문장이 끊기지 않고 계속 쌓여야 한다.
...
const allSentences = computed(() => {
  const flat = questions.value.flatMap((q, qi) => {
    // 실측 STT 자막(q.captionSegments)이 있으면 그 문장·타이밍을 그대로 쓴다 —
    // start는 답변 오디오 기준 절대 초(TTS 구간 제외)라, 영상 실제 재생 시각과
    // 맞추려면 이 질문까지 누적된 TTS 재생 시간(ttsShiftAt)을 더해야 한다.
    if (q.captionSegments.length) {
      const shift = ttsShiftAt(qi)
      return q.captionSegments
        .filter((s) => s.text !== DUMMY_CAPTION)
        .map((s) => ({ atSec: Math.round(s.start) + shift, text: s.text, questionIndex: qi }))
    }
    return []
  })
...
// 녹화한 세션) 동작한다.
const activeTtsWindowIndex = computed(() => {
  const sec = absoluteVideoSec.value
  if (sec == null) return -1
  return questions.value.findIndex((_, qi) => {
    const range = ttsWindowVideoRangeAt(qi)
    return range && sec >= range.start && sec < range.end
  })
})
const activeSentence = computed(() => {
  if (activeTtsWindowIndex.value >= 0) return '질문을 읽어주는 중입니다.'
...
const activeQuestionIndex = computed(() => {
  const sec = absoluteVideoSec.value ?? questionStartSec.value
  return questionIndexAtTime(questions.value, sec)
})
const activeQuestionItem = computed(() => questions.value[activeQuestionIndex.value] ?? current.value)
  ...
  <main class="archive-report-shell metric-report-shell">
    ...
    <section class="archive-report-summary" aria-label="면접 정보와 분석 결과">
      ...
      <div class="archive-report-metrics">
        <header>
          <div><span>면접 결과</span><strong>{{ totalScore == null ? '—' : `${totalScore}점` }}</strong></div>
        </header>
        <dl>
          <div v-for="(card, i) in scoreCards" :key="`${card.label}-${i}`" class="archive-score-metric" tabindex="0">
            ...
            <dd>{{ card.value == null ? '—' : `${card.value}점` }}</dd>
            <aside class="archive-score-detail">
              ...
              <dl class="archive-score-breakdown">
                <div v-for="[rowLabel, rowValue] in card.rows" :key="rowLabel">
                  <dt v-if="rowLabel === '필러'"><span class="iv-term-hint" tabindex="0">필러<span class="iv-term-hint-bubble">"음", "어", "그"처럼 다음 말을 생각하는 동안 공백을 채우기 위해 사용하는 표현</span></span></dt>
    ...
    <section class="iv-metric-tabs" aria-label="음성·몸짓 지표">
      ...
      <div v-if="metricTab === 'voice' && currentVoicePace" class="iv-pace-panel">
        ...
        >
          ...
          <button
            ...
            :aria-label="`필러 '${f.word}' ${absClock(f.atSec)} · 영상 이동`"
            @click="seekVoice(f.atSec)"
          ></button>
          <div
            v-if="wasQuestionCutShort"
            class="iv-pace-cutoff"
            role="note"
            aria-label="이 지점에서 다음 질문으로 넘어감"
            :style="{ left: `${paceCutoffPct}%` }"
          >
        ...
        <div class="iv-pace-chips">
          <span class="iv-pace-chip is-filler"><span class="iv-term-hint" tabindex="0">필러<span class="iv-term-hint-bubble">"음", "어", "그"처럼 다음 말을 생각하는 동안 공백을 채우기 위해 사용하는 표현</span></span> {{ currentVoicePace.fillerTotal }}회<small>{{ currentVoicePace.fillerBreakdown.map(([w, n]) => `${w} ${n}회`).join(' · ') }}</small></span>
          <span class="iv-pace-chip is-silence">1초 이상 정적 {{ currentVoicePace.longSilenceCount }}회</span>
      ...
      <div v-else-if="metricTab === 'voice'" class="iv-metric-empty">질문별 음성 시계열 데이터가 없습니다.</div>
      ...
      <div v-else class="iv-metric-empty">질문별 몸짓 시계열 데이터가 없습니다.</div>
    </section>
      ...
      <div class="iv-label-col" :style="labelColHeight ? { height: `${labelColHeight}px` } : null">
        ...
        <div class="iv-caption-stack">
          ...
          <p
            ...
            :class="{ 'is-tts': activeTtsWindowIndex >= 0 }"
          >{{ activeSentence || '위 표시나 영상을 재생해보세요.' }}</p>
      ...
      <div class="iv-rq-layout">
        ...
        <div class="iv-rq-answer-col">
          <h3 class="iv-rq-q-title">
            ...
            <span class="iv-rq-q-title-score">{{ current.score == null ? '—' : `${current.score}점` }}</span>
          </h3>
