              ...
              <section class="presentation-panel iv-form-panel iv-field-card" aria-label="지원 맥락 설정">
                ...
                <div class="form-field iv-docs-field">
                  ...
                  <div class="iv-docs-actions">
                    ...
                    <button type="button" class="iv-doc-choice" :disabled="interview.docsSaving" @click="uploadDoc('resume')">자기소개서 등록</button>
                    <button type="button" class="iv-doc-choice" :disabled="interview.docsSaving" @click="uploadDoc('portfolio')">포트폴리오 등록</button>
                  </div>
                  ...
                  <ul v-if="selectedDocs.length" class="iv-docs-chosen">
                    <li v-for="doc in selectedDocs" :key="`${doc.kind}-${doc.id}`">
                      <div><strong>{{ doc.title }}</strong><small>{{ doc.label }}</small></div>
