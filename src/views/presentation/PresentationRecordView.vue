...
import {
  calculateWpm,
  countFillerWords,
  useRealtimePresentationAnalysis,
...
const gazeLabel = computed(() => (recording.stats.gazeHold == null ? '--' : recording.stats.gazeHold))
const postureLabel = computed(() => (recording.stats.posture == null ? '--' : recording.stats.posture))
const statusLabel = computed(() => (recording.isPaused ? '일시정지' : '녹화 중'))
const modelStatusLabel = computed(() => {
  if (analysis.modelStatus.value === 'running') return 'AI 분석 중'
  if (analysis.modelStatus.value === 'loading') return 'AI 모델 준비 중'
  if (analysis.modelStatus.value === 'error') return 'AI 분석 연결 오류'
  if (analysis.isReady.value) return 'AI 분석 준비 완료'
  return 'AI 분석 대기'
})
...
const onTick = () => {
  ...
  recording.setStats({
    ...
    gazeHold: analysis.gazeHold.value ?? analysis.gazeScore.value,
    posture: analysis.postureAverage.value ?? analysis.postureScore.value,
    voice: analysis.voice.value,
    voiceDb: analysis.voiceDb.value,
  })
}
...
const startPresentation = async () => {
  ...
  try {
    ...
    try {
      ...
      sessionError.value = '이 브라우저는 실시간 음성 인식을 지원하지 않아요. 영상 및 시선 및 자세 분석은 계속됩니다.'
    }
    await analysis.start(videoEl.value, stream.value)
...
const endRecording = async () => {
  ...
  try {
    ...
    await presentation.finishRecording({
      blob,
      durationSeconds,
      metrics,
      complete: !presentation.qnaEnabled,
    })
    await router.push(presentation.qnaEnabled ? '/presentation/qna' : '/presentation/analyzing')
  } catch (error) {
    ...
    <div class="record-shell">
      <aside class="record-rail">
        ...
        <section class="rail-section rail-analysis-section">
          <div class="rail-section-head">
            <span>실시간 분석</span>
            <span class="coach-model-state" :class="`is-${analysis.modelStatus.value}`">{{ modelStatusLabel }}</span>
          </div>
          <div class="rail-stat-grid">
            ...
            <div><strong>{{ gazeLabel }}</strong><small>시선 유지 %</small></div>
            <div><strong>{{ postureLabel }}</strong><small>자세 안정 점수</small></div>
          </div>
          <p class="coach-audio-state">
            음성 크기 <strong>{{ recording.stats.voice }}</strong>
            <span v-if="recording.stats.voiceDb != null">{{ recording.stats.voiceDb }} dB</span>
          </p>
        </section>
      </aside>
