...
import { createFileFormData, createJsonRequestFormData, createRecordingFormData } from '../../src/api/formData.js'
...
describe('API FormData helpers', () => {
  ...
  it('creates multipart requests with a JSON request part and optional files', async () => {
    const profileImage = new File(['avatar'], 'avatar.png', { type: 'image/png' })
    const formData = createJsonRequestFormData(
      { nickname: 'AIVO', removeProfileImage: false },
      { profileImage, ignored: null },
    )

    expect(formData.get('request').type).toBe('application/json')
