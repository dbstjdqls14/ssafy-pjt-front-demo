...
describe('presentationStore', () => {
  test('rejects an emoji practice title before creating a presentation', async () => {
    ...
    )).rejects.toThrow('한글, 영문, 숫자')

    expect(create).not.toHaveBeenCalled()
  })

  test('rejects invalid presentation descriptions before creating a presentation', async () => {
    const create = vi.spyOn(presentationApi, 'create').mockRejectedValue(new Error('network'))
    const practice = usePracticeStore()
    practice.setFolder({ id: '77', name: '발표 폴더' })
    const store = usePresentationStore()
    store.setTitle('발표 연습')
    store.setDescription('설명😀')
  ...
  test('rejects invalid slide notes before the bulk save API', async () => {
    const updateDescriptions = vi.spyOn(presentationApi, 'updateDescriptions')
    const store = usePresentationStore()
    store.sessionId = 31
    store.setSlides([{ slideId: 1, imageUrl: '/slide.png', description: '핵심😀' }])

    await expect(store.saveSlideNotes()).rejects.toThrow('이모지')
    expect(updateDescriptions).not.toHaveBeenCalled()
  })

  test('saves multilingual slide notes and PPT punctuation through the bulk API', async () => {
    const updateDescriptions = vi.spyOn(presentationApi, 'updateDescriptions').mockResolvedValue({})
    const store = usePresentationStore()
    store.sessionId = 31
    const description = '‘오늘의 건수’ ··· 日本語\t© ♥ ℃ →'
    store.setSlides([{ slideId: 1, imageUrl: '/slide.png', description }])

    await store.saveSlideNotes()

    expect(updateDescriptions).toHaveBeenCalledWith(31, [{ slideId: 1, description }])
  })
...
describe('interviewStore', () => {
  test('rejects an emoji practice title before creating an interview', async () => {
    ...
    await expect(store.createInterview()).rejects.toThrow('한글, 영문, 숫자')

    expect(create).not.toHaveBeenCalled()
  })

  test('rejects invalid interview descriptions before creating an interview', async () => {
    const create = vi.spyOn(interviewApi, 'create').mockRejectedValue(new Error('network'))
    const practice = usePracticeStore()
    practice.setFolder({ id: '77', name: '면접 폴더' })
    const store = useInterviewStore()
    store.setInfo({ title: '면접 연습', description: '설명😀' })
  ...
  test('does not call the add-question API for invalid question text', async () => {
    const addQuestion = vi.spyOn(interviewApi, 'addQuestion').mockResolvedValue({ questionId: 99 })
    const store = useInterviewStore()
    store.interviewId = 21

    await expect(store.addQuestion('질문😀')).rejects.toThrow('이모지')
    expect(addQuestion).not.toHaveBeenCalled()
  })

  test('sends multilingual non-emoji question text to the add-question API', async () => {
    const question = '‘질문’ ··· 日本語\t© ♥ ℃ →'
    ...
    expect(addQuestion).toHaveBeenCalledWith(21, question)
  })
