...
describe('presentation file validation', () => {
  test('accepts PDF/PPTX and rejects unsupported or oversized files', () => {
    expect(MAX_PRESENTATION_FILE_SIZE).toBe(50 * 1024 * 1024)
    ...
    expect(validatePresentationFile(oversized)).toContain('50MB')
  })
})
...
describe('realtime speech metrics', () => {
  ...
  test('calculates Korean syllables per second with the same formula as interview live analysis', () => {
    expect(calculateSyllablesPerSecond('안녕하세요 반갑습니다', 4)).toBe(2.5)
    expect(calculateSyllablesPerSecond('English only', 4)).toBe(0)
    expect(calculateSyllablesPerSecond('안녕', 0)).toBe(2)
  })
