...
describe('FolderSelectView folder picking', () => {
  ...
  test('keeps invalid folder text visible and counts it by grapheme without creating a folder', async () => {
    const { wrapper, practice } = await mountView()
    const createFolder = vi.spyOn(practice, 'createFolder')
    await wrapper.get('.folder-mode-tabs button:last-child').trigger('click')

    const name = wrapper.get('#newFolderName')
    const description = wrapper.get('#newFolderDesc')
    expect(name.attributes('maxlength')).toBeUndefined()
    expect(description.attributes('maxlength')).toBeUndefined()
    await name.setValue('발표 폴더👨‍👩‍👧‍👦')
    await description.setValue('폴더 설명😀')

    expect(name.element.value).toBe('발표 폴더👨‍👩‍👧‍👦')
    expect(description.element.value).toBe('폴더 설명😀')
    expect(wrapper.get('[data-testid="folder-name-counter"]').text()).toBe('6/20')
    expect(wrapper.get('[data-testid="folder-description-counter"]').text()).toBe('6/100')
    expect(wrapper.findAll('.folder-new-form .field-error')).toHaveLength(2)

    await wrapper.get('.workflow-side-next').trigger('click')
    expect(createFolder).not.toHaveBeenCalled()
  })

  test('caps new folder fields at their visible grapheme limits', async () => {
    const { wrapper } = await mountView()
    await wrapper.get('.folder-mode-tabs button:last-child').trigger('click')
    const name = wrapper.get('#newFolderName')
    const description = wrapper.get('#newFolderDesc')

    await name.setValue('가'.repeat(21))
    await description.setValue('日'.repeat(101))

    expect(name.element.value).toBe('가'.repeat(20))
    expect(description.element.value).toBe('日'.repeat(100))
    expect(wrapper.get('[data-testid="folder-name-counter"]').text()).toBe('20/20')
    expect(wrapper.get('[data-testid="folder-description-counter"]').text()).toBe('100/100')
  })
