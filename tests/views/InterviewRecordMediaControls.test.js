...
describe('InterviewRecordView device controls', () => {
  beforeEach(() => {
    ...
    media.requestVideo.mockImplementation(async () => {
      media.videoTrack.readyState = 'live'
      media.videoTrackRef.value = media.videoTrack
      return media.videoTrack
    })
    media.requestAudio.mockImplementation(async () => {
      media.audioTrack.readyState = 'live'
      media.audioTrackRef.value = media.audioTrack
      return media.audioTrack
    })
    media.requestRequiredDevices.mockImplementation(async () => {
      media.videoTrackRef.value = media.videoTrack
      media.audioTrackRef.value = media.audioTrack
    })
    media.releaseVideo.mockImplementation(() => { media.videoTrackRef.value = null })
    media.releaseAudio.mockImplementation(() => { media.audioTrackRef.value = null })
  ...
  test('사이트 내부에서 장치를 끈 상태로 면접 시작을 눌러도 장치를 다시 요청하지 않는다', async () => {
    const { wrapper } = await mountView()
    const [cameraButton, microphoneButton] = wrapper.findAll('.ivr-toggle')
    const deviceRequestsBeforeStart = media.requestRequiredDevices.mock.calls.length
    ...
    const warning = wrapper.get('[data-testid="interview-device-off-warning"]')
    expect(warning.text()).toContain('카메라와 마이크가 꺼져 있습니다')
    expect(warning.text()).toContain('카메라와 마이크를 직접 켠 뒤 면접 시작을 다시 눌러 주세요.')
    expect(media.requestRequiredDevices).toHaveBeenCalledTimes(deviceRequestsBeforeStart)
    expect(cameraButton.attributes('aria-pressed')).toBe('false')
    expect(microphoneButton.attributes('aria-pressed')).toBe('false')
