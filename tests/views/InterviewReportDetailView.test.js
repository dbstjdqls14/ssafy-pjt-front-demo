...
describe('InterviewReportDetailView real report fields', () => {
  ...
  it('places the next-question marker at the real question boundary in both metric tabs', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const router = createRouter({
      history: createMemoryHistory(),
      ...
      questionEvaluations: [
        {
          ...
          endTimeSeconds: 58,
          durationSeconds: 58,
          segments: [{ text: 'first answer', startTimeMs: 0, endTimeMs: 58000 }],
          voicePace: {
            avgPace: 2.6,
            buckets: [{ startSec: 30, endSec: 40, pace: 2.6 }],
            slowest: { startSec: 30, endSec: 40, pace: 2.6 },
            fastest: { startSec: 30, endSec: 40, pace: 2.6 },
            fillerEvents: [],
            silences: [],
          },
          gestureSeries: {
            buckets: [{ startSec: 30, endSec: 40, tiltPct: 10 }],
            gazeEvents: [],
          },
        },
        {
          ...
          endTimeSeconds: 118,
          durationSeconds: 60,
          segments: [{ text: 'second answer', startTimeMs: 58000, endTimeMs: 118000 }],
        },
      ],
    }
    ...
    expect(wrapper.get('.iv-pace-cutoff').attributes('style')).toContain('left: 100%')

    await wrapper.get('.iv-metric-tab:nth-child(2)').trigger('click')

    expect(wrapper.get('.iv-pace-cutoff').attributes('style')).toContain('left: 100%')
  })

  it('does not show a next-question marker for the final question', async () => {
    const pinia = createPinia()
    setActivePinia(pinia)
    const router = createRouter({
      history: createMemoryHistory(),
    ...
    interview.report = {
      ...
      questionEvaluations: [{
        ...
        voicePace: {
          avgPace: 2.4,
          buckets: [{ startSec: 20, endSec: 30, pace: 2.4 }],
          slowest: { startSec: 20, endSec: 30, pace: 2.4 },
          fastest: { startSec: 20, endSec: 30, pace: 2.4 },
          fillerEvents: [],
          silences: [],
        },
      }],
    ...
    expect(wrapper.find('.iv-pace-cutoff').exists()).toBe(false)
  })
