...
describe('MyPageDocumentsView registration', () => {
  ...
  test('selects a document type and uploads an explicitly titled PDF', async () => {
    ...
    expect(wrapper.get('[data-testid="document-title"]').attributes('maxlength')).toBeUndefined()
  ...
  test('rejects an empty title and non-PDF without calling the store', async () => {
    ...
    expect(wrapper.get('[data-testid="document-title-error"]').text()).toContain('제목')
    expect(upload).not.toHaveBeenCalled()
  })

  test('keeps an invalid document title visible with a grapheme counter and blocks upload', async () => {
    const { wrapper, store } = await mountView()
    const upload = vi.spyOn(store, 'uploadDocument')
    ...
    await title.setValue('포트폴리오👨‍👩‍👧‍👦')

    expect(title.element.value).toBe('포트폴리오👨‍👩‍👧‍👦')
    expect(wrapper.get('[data-testid="document-title-counter"]').text()).toBe('6/50')
    expect(wrapper.get('[data-testid="document-title-error"]').text()).toContain('한글, 영문, 숫자')

    await wrapper.get('[data-testid="submit-document"]').trigger('submit')
    expect(upload).not.toHaveBeenCalled()
  })

  test('caps a document title at 50 visible graphemes', async () => {
    const { wrapper } = await mountView()
    await wrapper.get('[data-testid="open-document-registration"]').trigger('click')
    await wrapper.get('[data-document-type="resume"]').trigger('click')
    const title = wrapper.get('[data-testid="document-title"]')

    await title.setValue('가'.repeat(51))

    expect(title.element.value).toBe('가'.repeat(50))
    expect(wrapper.get('[data-testid="document-title-counter"]').text()).toBe('50/50')
  })
