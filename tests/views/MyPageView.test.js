...
describe('MyPageView profile image editing', () => {
  ...
  test('loads the server profile before initializing a direct edit draft', async () => {
    ...
    expect(wrapper.get('#nickname').attributes('maxlength')).toBeUndefined()
  })
  ...
  test('keeps invalid nickname text visible with a grapheme counter until it is corrected', async () => {
    const { wrapper, updateProfile } = await mountView()
    await wrapper.get('.mypage-primary-action').trigger('click')
    const nickname = wrapper.get('#nickname')

    await nickname.setValue('사용자👨‍👩‍👧‍👦')

    expect(nickname.element.value).toBe('사용자👨‍👩‍👧‍👦')
    expect(wrapper.get('.mypage-edit-form .field-counter').text()).toBe('4/20')
    expect(wrapper.get('.mypage-edit-form .field-error').text()).toContain('한글, 영문, 숫자, 밑줄')
    await wrapper.get('form').trigger('submit')
    expect(updateProfile).not.toHaveBeenCalled()

    await nickname.setValue('사용자_1')
    expect(wrapper.find('.mypage-edit-form .field-error').exists()).toBe(false)
  })

  test('caps edited nickname input at 20 visible graphemes', async () => {
    const { wrapper } = await mountView()
    await wrapper.get('.mypage-primary-action').trigger('click')
    const nickname = wrapper.get('#nickname')

    await nickname.setValue(`${'가'.repeat(20)}나`)

    expect(nickname.element.value).toBe('가'.repeat(20))
    expect(wrapper.get('.mypage-edit-form .field-counter').text()).toBe('20/20')
  })
