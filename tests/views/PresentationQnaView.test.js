...
const mountView = async (questions = [{ id: 201, content: 'AIVO의 차별점은 무엇인가요?' }]) => {
  ...
  await flushPromises()
  return { wrapper, router, presentation, recording: useRecordingStore() }
}
...
describe('PresentationQnaView answer persistence', () => {
  ...
  test('blocks route navigation and preserves the current Q&A when the user keeps answering', async () => {
    ...
    expect(router.currentRoute.value.path).toBe('/presentation/qna')
    expect(wrapper.get('[data-testid="presentation-qna-exit-dialog"]').text())
      .toContain('지금 나가면 답변과 발표 기록이 저장되지 않으며 리포트도 생성되지 않습니다.')

    await wrapper.get('[data-testid="continue-presentation-qna"]').trigger('click')

    expect(wrapper.find('[data-testid="presentation-qna-exit-dialog"]').exists()).toBe(false)
    expect(router.currentRoute.value.path).toBe('/presentation/qna')
  ...
  test('discards Q&A recording state and follows the original destination after confirmation', async () => {
    const { wrapper, router, presentation, recording } = await mountView()
    recording.start()
    recording.tick()
    recording.addTranscript('폐기할 답변')

    await router.push('/archive')
    await flushPromises()
    await wrapper.get('[data-testid="discard-presentation-qna"]').trigger('click')
    await flushPromises()
