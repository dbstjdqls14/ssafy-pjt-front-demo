...
describe('PresentationSetupView', () => {
  ...
  test('keeps invalid presentation text visible with grapheme counters and policy guidance', async () => {
    const { pinia, router } = await mountSetup()
    const wrapper = mount(PresentationSetupView, { global: { plugins: [pinia, router] } })

    await wrapper.get('#title').setValue('서비스 소개 👨‍👩‍👧‍👦')
    await wrapper.get('#description').setValue('발표 설명😀')

    expect(wrapper.get('#title').element.value).toBe('서비스 소개 👨‍👩‍👧‍👦')
    expect(wrapper.get('#description').element.value).toBe('발표 설명😀')
    expect(wrapper.get('[data-testid="presentation-title-counter"]').text()).toBe('8/15')
    expect(wrapper.get('[data-testid="presentation-description-counter"]').text()).toBe('6/100')
    expect(wrapper.get('.form-field.field-invalid .field-error').text()).toContain('한글, 영문, 숫자')
  })

  test('caps setup text by graphemes and allows multilingual non-emoji descriptions', async () => {
    const { pinia, router } = await mountSetup()
    const wrapper = mount(PresentationSetupView, { global: { plugins: [pinia, router] } })
    ...
    expect(wrapper.get('[data-testid="presentation-title-counter"]').text()).toBe('15/15')
    expect(wrapper.get('[data-testid="presentation-description-counter"]').text()).toBe('100/100')

    await description.setValue('‘오늘의 건수’ ··· 日本語\t© ♥ ℃ →')
    expect(description.element.value).toBe('‘오늘의 건수’ ··· 日本語\t© ♥ ℃ →')
